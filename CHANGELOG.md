...
- 修复 3ds Max 插件启动器生成器的 Python 字符串语法错误，恢复完整测试导入。
- 发布版本提升至 2.1.5，避免将未通过 Compatibility 的 2.1.4 标记提交作为公开版本。

### Verified

- 8 个 GameArt 插件安装/卸载冒烟测试仍保持 8/8 通过。
- Windows GameArt 安装器构建与 UI 截图流程已通过。
